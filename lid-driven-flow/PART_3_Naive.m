...
clc;
method='Naive';
Nv=10;             %Number of control volumes
delta = 1/Nv;
...
while t<t_end && ~steady_state
 ...
 P_pred=ADI_helper_naive(P_pred,u_pred,v_pred,rho,dt,delta,N);
 ...
 if(err<0.1 || norm((u_curr-u_temp) + (v_curr-v_temp))<1e-03)
...
naive_50X50=containers.Map({'u','v','P','x','y'},{u_curr,v_curr,P_curr,x,y});
save('naive_50X50.mat','naive_50X50');
