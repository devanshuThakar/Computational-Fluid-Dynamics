...
for i=2:N-1
    for j=2:N-1
          if(j==2) %West boundary
             AW(i,j)=(rho*x(j-1)*delta) -Gamma*2;
             AP(i,j)=AP(i,j)+Gamma*2;
          else
              AW(i,j)=((-rho*x(j-1)*delta)/2)-Gamma;
              AP(i,j)=AP(i,j)-AW(i,j);
          end
          if(i==N-1) %North boundary
              AN(i,j)=(-rho*y(i+1)*delta)-Gamma*2;
              AP(i,j)=AP(i,j)+Gamma*2;
          else
              AN(i,j)=((-rho*y(i+1)*delta)/2)-Gamma;
              AP(i,j)=AP(i,j)-AN(i,j);
          end
          if(j==N-1) %East boundary
              AE(i,j)=0;
              AP(i,j)=AP(i,j)+rho*x(j+1)*delta;
          else
              AE(i,j)=((rho*x(j+1)*delta)/2) -Gamma;
              AP(i,j)=AP(i,j)-AE(i,j);
          end
          if(i==2) %South boundary
              AS(i,j)=0;
              AP(i,j)=AP(i,j)+rho*y(i-1)*delta;
          else
              AS(i,j)=((rho*y(i-1)*delta)/2)-Gamma;  
...
while iter<10000 && err > 1e-5
    for j=2:N-1
        for i=N-1:-1:2
            ...
            if(j==N-1)
                phi_curr(i,N)=phi_curr(i,N-1);
            end
        end
        if(i==2)
            phi_curr(1,2:N)=phi_curr(2,2:N);
        end
    end
