% Implementation of Implicit scheme with ADI to solve time dependent 2D problems %
clear all;
clc
...
while ~done && t < te
    ...
    %phi_temp = zeros(N,M);
     ...
     for i=2:N-1
         Ap = zeros(M-2,1)-(4+(2*delta_x^2/delta_t));
         Ae = zeros(M-2-1,1)+1;
         Aw = zeros(M-2-1,1)+1;
         B = source_arr(i,2:M-1)*delta_x^2 ...
             - (delta_x^2)*2*phi_num(i,2:M-1,prev)/(delta_t) ...
             - phi_num(i+1,2:M-1,prev) - phi_num(i-1,2:M-1,prev);
     ...
     for j=2:M-1
         Ap = zeros(N-2,1)-(4+(2*delta_x^2/delta_t));
         Ae = zeros(N-2-1,1)+1;
         Aw = zeros(N-2-1,1)+1;
         B = source_arr(2:N-1,j)*delta_x^2  - (delta_x^2)*2*phi_temp(2:N-1,j)/(delta_t) - phi_temp(2:N-1,j+1) - phi_temp(2:N-1,j-1);
