...
%     done = true;
    ...
    for i=2:N-1
        for j=2:N-1
            phi_num(i,j,curr) = (phi_num(i+1,j,prev) + phi_num(i-1,j,prev) + phi_num(i,j+1,prev)+phi_num(i,j-1,prev) - source_arr(i,j)*Delta^2)/4;
            % Updating the previous phi as it is Gauss Seidel Method
            phi_num(i,j,prev) = phi_num(i,j,curr);
        end
    end
...
save(sprintf('residual_%i_%s', N,method), 'residual');
save(sprintf('phi_num_%i_%s',N, method), 'phi_numerical', 'x', 'y');